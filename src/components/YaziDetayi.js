...
const handleDelete=(commentid)=>{
  ...
  .then(()=>{
    console.log("yazı detayı içinde silme işlemi ve state güncellemesi")
      ...
      close();
      //state güncelle
      api().get(`/posts/${id}/comments`).then((response)=>{setYorumlar(response.data)})
      //push
 ...
 //axios.all veya Promise.all
  ...
  <Link className="ui blue basic button" to={`/posts/${yaziDetayi.id}/edit`}>Düzenle</Link>
  {/* <button className="ui  red basic button">Sil</button> */}
  <SilModal yazi={yaziDetayi} push={history.push}></SilModal>
</div>
      <p>{yaziDetayi.content}</p>
      <YaziYorumları open={open} hata={hata} show={show} close={close} handleDelete={handleDelete} yaziid={yaziDetayi.id} yorumlar={yorumlar} handleCommentSubmit={handleCommentSubmit}></YaziYorumları>
      {/* <h3>Yorumlar</h3>
      {yorumlar.map((yorum) => {
        return (
